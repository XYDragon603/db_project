...
SELECT setval(pg_get_serial_sequence('medication_catalog', 'catalog_id'),
              COALESCE((SELECT MAX(catalog_id) FROM medication_catalog), 1), true);
SELECT setval(pg_get_serial_sequence('medication_brands', 'brand_id'),
              COALESCE((SELECT MAX(brand_id) FROM medication_brands), 1), true);
